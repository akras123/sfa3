(function($){
  ...
  $('.element.bestuursleden .blogList > ul > li').each(function(i) {
    var $this = $(this),
        $postFeaturedImage = $this.find('.postFeaturedImage img'),
        $imageAnchor = $this.find('.postFeaturedImage a'),
        $postTitle   = $this.find('.postTitle'),
        $postFunctie = $this.find('.postBody > h3'),
        $postBodyModal = $this.find('.postBody .modal');
